...
def stage1_checks(mod):
    ...
    if tfails:
        return False, tfails
    # matoulib/gigafauna share one seam-map (matoulib/ARCHITECTURE.md, generated by matoulib_arch.py)
    # of the matoulib source tree — mirrors GATE 9 (arch_map.py --check) in release_pack.py, which
    # refuses a pack release if the tooling data-dependency map is stale. Scoped by mod name so it
    # only gates a release that actually includes matoulib/gigafauna (an OaT-only run isn't blocked
    # by a matoulib-only map).
    if mod["name"].lower() in ("matoulib", "gigafauna"):
        _, code = run(["python3", os.path.join(SCRIPT_DIR, "matoulib_arch.py"), "--check"], check=False)
        if code != 0:
            return False, ["matoulib_arch.py --check: seam map stale (run "
                           "`python3 matoulib_arch.py` in Cat-Pack-Utilities to regenerate)"]
