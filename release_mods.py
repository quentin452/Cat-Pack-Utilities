...
Per mod, with SAFETY GATES first (nothing is released if a gate fails):
  ...
  3b. SEAM-MAP pass          (matoulib/gigafauna only: matoulib_arch.py --check — mirrors GATE 9
                             (arch_map.py --check) in release_pack.py, scoped to mods backed by the
                             matoulib repo so an OaT-only release isn't blocked by a stale seam map)
...
def stage1_checks(mod):
    ...
    if tfails:
        return False, tfails
    # Post-split, each matou repo has its OWN seam-map (<repo>/ARCHITECTURE.md, generated by
    # matoulib_arch.py --repo <name>) — mirrors GATE 9 (arch_map.py --check) in release_pack.py, which
    # refuses a pack release if the tooling data-dependency map is stale. Scoped by mod name so it only
    # gates a release that actually includes matoulib-core/gigafauna (an OaT-only run isn't blocked).
    seam_repo = {"gigafauna": "gigafauna"}.get(mod["name"].lower(), "matoulib-core")
    if mod["name"].lower() in ("matoulib", "matoulib-core", "gigafauna"):
        _, code = run(["python3", os.path.join(SCRIPT_DIR, "matoulib_arch.py"),
                       "--repo", seam_repo, "--check"], check=False)
        if code != 0:
            return False, [f"matoulib_arch.py --repo {seam_repo} --check: seam map stale (run "
                           f"`python3 matoulib_arch.py --repo {seam_repo}` in Cat-Pack-Utilities)"]
