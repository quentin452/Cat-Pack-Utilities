...
Usage:
  ...
  reserve_slugs.py --execute             # create the missing Modrinth drafts (outward)
  reserve_slugs.py --cf-checklist        # print the manual CurseForge creation checklist
...
def main():
    ...
    if cf_ids and args.wire_manifest and not (args.execute or args.verify):
    ...
    print(f"=== Modrinth {'CREATE (--execute)' if args.execute else 'DRY-RUN'} ===")
    created = {}
    for n in names:
        spec = PROJECTS[n]
        ...
        if exists is None:
            ...
            continue
        created[n] = create_modrinth_project(spec, token, dry_run=not args.execute)

    if args.cf_checklist or not args.execute:
        print_cf_checklist(names)
